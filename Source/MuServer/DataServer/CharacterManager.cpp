...
#include "Util.h"
#include <algorithm> // Adicione esta linha no início do arquivo
...
bool CCharacterManager::GetCharacterInfo(CHARACTER_INFO* lpCharacterInfo, char* name)
{
    ...
    // Substitua a linha problemática por esta
    std::transform(nme.begin(), nme.end(), nme.begin(), [](unsigned char c){ return std::tolower(c); });
